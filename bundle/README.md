...
- `default-deny-egress.yaml` — applied to every namespace before
  ...
  Allows in-cluster pods, DNS and the API server (by node IP — the Jenkins
  config-reload sidecar lists ConfigMaps; without this rule Jenkins never
  leaves `Init`). The shipped `cf-config` policy needs the same rule.
